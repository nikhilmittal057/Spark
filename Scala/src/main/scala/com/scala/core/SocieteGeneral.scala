...
object SocieteGeneral extends App {
  ...
  val Nodes_degrees = src_from_df.union(dest_to_df).groupBy("vertices").agg(sum("connecting_count").alias("degree")).cache()

  val e = graphDF.count()
  val n = Nodes_degrees.count()
  val AverageDegree =  2*(e/n)

  val finalDF=  Nodes_degrees.where($"degree" >= AverageDegree).cache()
